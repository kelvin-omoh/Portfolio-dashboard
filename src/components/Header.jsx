      ...
      <div className="container mx-auto px-4 md:px-6 py-3 md:py-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 md:space-x-5">
            <div className={`w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center transition-all duration-300 ${isDarkMode
              ? 'bg-white text-black shadow-lg'
              : 'bg-black text-white shadow-lg'
              }`}>
              <span className="font-bold text-lg md:text-2xl font-ivy">R</span>
            </div>
            <div>
              <h1 className={`text-lg md:text-3xl font-bold font-ivy ${isDarkMode ? 'text-white' : 'text-black'}`}>
                RicardianCorp
              </h1>
              <p className="text-xs md:text-sm font-satoshi opacity-70 mt-0.5 md:mt-1 hidden sm:block">Professional HFT Platform • NSE & Crypto Markets • Ultra-Low Latency</p>
              <p className="text-xs font-satoshi opacity-70 mt-0.5 sm:hidden">HFT Trading Platform</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 md:space-x-8">
            <div className="text-center">
              <p className="text-xs md:text-sm opacity-70 font-satoshi mb-1">Portfolio Value</p>
              <p className={`text-lg md:text-2xl font-bold font-satoshi ${isDarkMode ? 'text-white' : 'text-black'}`}>{formatCurrency(portfolio.totalValue)}</p>
            </div>
            <div className="text-center hidden sm:block">
              <p className="text-xs md:text-sm opacity-70 font-satoshi mb-1">Daily Change</p>
              <p className={`text-sm md:text-xl font-semibold font-satoshi ${portfolio.dailyChange >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                {portfolio.dailyChange >= 0 ? '+' : ''}{formatCurrency(portfolio.dailyChange)}
              </p>
            </div>
            <div className="text-center">
              <p className="text-xs md:text-sm opacity-70 font-satoshi mb-1">Change %</p>
              <p className={`text-sm md:text-xl font-semibold font-satoshi ${portfolio.dailyChangePercent >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          <div className="flex items-center space-x-3 md:space-x-6">
            ...
            <button
              onClick={toggleTheme}
              className={`p-2 md:p-3 rounded-lg md:rounded-xl transition-all duration-300 ${isDarkMode
